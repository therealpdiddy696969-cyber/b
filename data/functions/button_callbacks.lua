...
G.FUNCS.can_continue = function(e)
  if e.config.func then --refers to this function, or 'can_continue', so this doesn't run repeatedly
    ...
    local savefile = G.FILES[G.SETTINGS.profile..'/'..'save.jkr']
    if savefile == nil then
        e.config.colour = G.C.UI.BACKGROUND_INACTIVE
        e.config.button = nil
    else
      if not G.SAVED_GAME then 
        G.SAVED_GAME = savefile
      end
...
G.FUNCS.load_profile = function(delete_prof_data)
  ...
  G.E_MANAGER:add_event(Event({
    ...
    func = function()
      G:delete_run()
      local _name = nil
      G.SETTINGS.profile = G.focused_profile or 1
      ...
      G.PROFILES[G.focused_profile].deleted = nil
...
G.FUNCS.can_delete_profile = function(e)
  if not G.CHECK_PROFILE_DATA then
    local _deleted, _version = check_profile(G.FILES[G.focused_profile..'/profile.jkr'])
    G.CHECK_PROFILE_DATA = not _deleted
  end
...
G.FUNCS.delete_profile = function(e, bypass_warning)
  local warning_text = bypass_warning or e.UIBox:get_UIE_by_ID('warning_text')
  if not bypass_warning and (warning_text.config.colour ~= G.C.WHITE) then 
    ...
    love.mod_filesystem.remove(G.focused_profile..'/'..'save.jkr')
    love.mod_filesystem.remove(G.focused_profile..'/'..'meta.jkr')
    love.mod_filesystem.remove(G.focused_profile..'/'..'unlock_notify.jkr')
    love.mod_filesystem.remove(G.focused_profile..'')
    G.FILES[G.focused_profile..'/'..'save.jkr'] = nil
    G.FILES[G.focused_profile..'/'..'meta.jkr'] = nil
    G.FILES[G.focused_profile..'/'..'unlock_notify.jkr'] = nil
    G.SAVED_GAME = nil
    G.DISCOVER_TALLIES = nil
    G.PROGRESS = nil
    G.SETTINGS.profile_counters[G.focused_profile] = (G.SETTINGS.profile_counters[G.focused_profile] or 0) + 1

    G:empty_profile(G.focused_profile)
    
    if not bypass_warning then 
...
G.FUNCS.text_input = function(e)
  ...
  if G.F_MOBILE then
    ...
    if G.CONTROLLER.text_input_hook == e and (G.F_MOBILE or G.CONTROLLER.HID.controller) then
...
G.FUNCS.apply_window_changes = function(_initial)
  ...
  love.window.updateMode(
    ...
    {fullscreen = G.SETTINGS.WINDOW.screenmode ~= 'Windowed',
      ...
      vsync = G.SETTINGS.WINDOW.vsync == 1 and 1 or 0,
      resizable = true,
      display = G.SETTINGS.WINDOW.selected_display,
      highdpi = (love.system.getOS() == 'OS X') or (love.system.getOS() == 'iOS') or width < 1024
    })
...
G.FUNCS.link_tree = function(e)
  G.SETTINGS.paused = true
  G.FUNCS.overlay_menu{
    definition = G.UIDEF.link_tree(),
  }
end
...
G.FUNCS.gamecenter_login = function(e)
  if love.platform and love.platform.authenticateLocalPlayer then
    love.platform.authenticateLocalPlayer()
  end
end

G.FUNCS.rate_us_flow =  function(e)
  if not G.SETTINGS.rated then 
    if love.platform and love.platform.requestReview then
      love.platform.requestReview()
    end
    G.SETTINGS.rated = true
    G:save_settings()
  end
end
...
G.FUNCS.profile_select = function(e)
  ...
  for i = 1, 3 do
    if i ~= G.focused_profile and love.mod_filesystem.getInfo(i..'/'..'profile.jkr') then G:load_profile(i) end
  end
  G:load_profile(G.focused_profile)
...
--G.FUNCS.go_to_gamecenter = function(e)
...
G.FUNCS.go_to_faq = function(e)
  love.system.openURL( "https://www.playbalatro.com/faq" )
end

G.FUNCS.go_to_how_to_play = function(e)
  love.system.openURL( "https://www.youtube.com/watch?v=p_V_h7i001g" )
end
...
G.FUNCS.retry_connection = function(e)
  G:load_all_files(true)
  G:delete_run()
...
G.FUNCS.continue_offline = function(e)
...
G.FUNCS.start_setup_run = function(e)
  ...
  elseif G.SETTINGS.current_setup == 'Continue' then
    if G.SAVED_GAME ~= nil then
      G.FUNCS.start_run(nil, {savetext = copy_table(G.SAVED_GAME)})
    end
  end
end
