      ...
      func = function()
        ...
        G.RESET_JIGGLES = true
            ...
            if not G.FTP_LOCKED then 
                ...
                if G.GAME.round_resets.ante >= 6 and G.GAME.blind:get_type() == 'Boss' then
                    game_won = true
                    G.GAME.won = true
                end
            end
            ...
            if game_over then
                ...
                analytic_event('game_over',
                    'demo_flag', not G.FTP_LOCKED,
                    'deck', G.GAME.selected_back.name or 'error',
                    'stake', G.GAME.stake or 'error',
                    'language', G.SETTINGS.language or 'error',
                    'ante',  G.GAME.round_resets.ante or -1,
                    'run_won', not not G.GAME.won)
                ...
                G.E_MANAGER:add_event(Event({
                    ...
                    func = function()
                        ...
                        if G.GAME.round_resets.blind and G.GAME.round_resets.blind.name == 'Small Blind' then
                            G.GAME.round_resets.blind_states.Small = 'Defeated'
                        elseif G.GAME.round_resets.blind and G.GAME.round_resets.blind.name == 'Big Blind' then
...
G.FUNCS.tutorial_controller = function()
    ...
    if not G.SETTINGS.paused and (not G.SETTINGS.tutorial_complete) then
        ...
        if G.SETTINGS.tutorial_progress.hold_parts['second_hand'] then
            set_cloud_flag('tutorial_complete')
        end
...
G.FUNCS.tutorial_part = function(_part)
    analytic_event('tutorial_step',
        'demo_flag', not G.FTP_LOCKED,
        'current_step', _part or 'error',
        'language', G.SETTINGS.language or 'error')
    ...
    elseif _part == 'big_blind' then 
        step = tutorial_info({
            ...
            hard_set = true,
            attach = {major =  G.HUD, type = 'cm', offset = {x = 0.7, y = -0.5}},
            step = step,
        })
        step = tutorial_info({
            text_key = 'bb_2',
            highlight = {
                G.blind_select.UIRoot.children[1].children[2].config.object:get_UIE_by_ID('blind_name'),
                G.blind_select.UIRoot.children[1].children[2].config.object:get_UIE_by_ID('tag_desc').parent,
            },
            disabled_buttons = {'skip_blind'},
            attach = {major =  G.HUD, type = 'cm', offset = {x = 0.7, y = -0.5}},
            step = step,
        })
        step = tutorial_info({
    ...
    elseif _part == 'first_hand_2' then
        step = tutorial_info({
            ...
            attach = {major = G.hand, type = 'cl', offset = {x = -1, y = -0.3}},
    ...
    elseif _part == 'shop_1' then
        ...
        step = tutorial_info({
            ...
            attach = {major = G.shop, type = 'tr', offset = {x = -0.5, y = 5}},
            no_button = true,
        ...
        step = tutorial_info({
            ...
            attach = {major = G.shop, type = 'tr', offset = {x = -4, y = 5}},
            step = step,
        })
        step = tutorial_info({
            text_key = 's_10',
