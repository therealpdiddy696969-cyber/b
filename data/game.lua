...
function Game:start_up()
    ...
    --Mods
    self.SETTINGS.handsize = tonumber(self.SETTINGS.handsize) or 8
...
function Game:init_item_prototypes()
    ...
    self.tag_undiscovered = self.tag_undiscovered or
    { name = 'Not Discovered', order = 1, config = { type = '' }, pos = { x = 3, y = 4 } }
    ...
    self.b_undiscovered = self.b_undiscovered or
    { name = 'Undiscovered', debuff_text = 'Defeat this blind to discover', pos = { x = 0, y = 30 } }
    ...
    self.j_locked = self.j_locked or
    { unlocked = false, max = 1, name = "Locked", pos = { x = 8, y = 9 }, set = "Joker", cost_mult = 1.0, config = {} }
    self.v_locked = self.v_locked or
    { unlocked = false, max = 1, name = "Locked", pos = { x = 8, y = 3 }, set = "Voucher", cost_mult = 1.0, config = {} }
    self.c_locked = self.c_locked or
    { unlocked = false, max = 1, name = "Locked", pos = { x = 4, y = 2 }, set = "Tarot", cost_mult = 1.0, config = {} }
    self.j_undiscovered = self.j_undiscovered or
    { unlocked = false, max = 1, name = "Locked", pos = { x = 9, y = 9 }, set = "Joker", cost_mult = 1.0, config = {} }
    self.t_undiscovered = self.t_undiscovered or
    { unlocked = false, max = 1, name = "Locked", pos = { x = 6, y = 2 }, set = "Tarot", cost_mult = 1.0, config = {} }
    self.p_undiscovered = self.p_undiscovered or
    { unlocked = false, max = 1, name = "Locked", pos = { x = 7, y = 2 }, set = "Planet", cost_mult = 1.0, config = {} }
    self.s_undiscovered = self.s_undiscovered or
    { unlocked = false, max = 1, name = "Locked", pos = { x = 5, y = 2 }, set = "Spectral", cost_mult = 1.0, config = {} }
    self.v_undiscovered = self.v_undiscovered or
    { unlocked = false, max = 1, name = "Locked", pos = { x = 8, y = 2 }, set = "Voucher", cost_mult = 1.0, config = {} }
    self.booster_undiscovered = self.booster_undiscovered or
    { unlocked = false, max = 1, name = "Locked", pos = { x = 0, y = 5 }, set = "Booster", cost_mult = 1.0, config = {} }
    ...
    self.P_CENTERS = self.P_CENTERS or {
        ...
        j_pursuer = { order = 152, unlocked = false, start_locked = true, demo = false, discovered = false, blueprint_compat = true, perishable_compat = true, eternal_compat = true, rarity = 3, cost = 10, name = "The Pursuer", pos = { x = 2, y = 0 }, set = "Joker", atlas="mod_jokers", effect = "", config = { extra = 1, x_mult = 1 }, unlock_condition = { type = 'common_jokers', extra = 5 } },
        ...
        b_black = { name = "Black Deck", stake = 1, demo = true, unlocked = false, start_locked = true, order = 5, pos = { x = 3, y = 2 }, set = "Back", config = { hands = -1, joker_slot = 5 }, unlock_condition = { type = 'discover_amount', amount = 100 } },
    ...
    -------------------------------------
    if not love.mod_filesystem.getInfo(G.SETTINGS.profile .. '') then love.mod_filesystem.createDirectory(G.SETTINGS
        .profile .. '') end
    ...
    for k, v in pairs(self.P_CENTERS) do
        ...
        if not v.wip and not v.demo then
            ...
            if not v.unlocked and (string.find(k, '^j_') or string.find(k, '^b_') or string.find(k, '^v_')) then self.P_LOCKED[#self.P_LOCKED + 1] =
                v end
    ...
    for k, v in pairs(self.P_CENTERS) do
        ...
        if not v.wip then
            if v.set and v.set ~= 'Joker' and not v.skip_pool and not v.omit then table.insert(
                self.P_CENTER_POOLS[v.set], v) end
    ...
    table.sort(self.P_CENTER_POOLS["Back"],
        function(a, b) return (a.order - (a.unlocked and 100 or 0)) < (b.order - (b.unlocked and 100 or 0)) end)
    ...
    table.sort(self.P_CENTER_POOLS["Demo"],
        function(a, b) return a.order + (a.set == 'Joker' and 1000 or 0) < b.order + (b.set == 'Joker' and 1000 or 0) end)
    for i = 1, 4 do
...
function Game:set_render_settings()
    ...
    for i = 1, #self.animation_atli do
        ...
        self.ANIMATION_ATLAS[self.animation_atli[i].name].image = love.graphics.newImage(self.animation_atli[i].path,
            { mipmaps = true, dpiscale = self.SETTINGS.GRAPHICS.texture_scaling })
...
function Game:delete_run()
    if self.ROOM then
        ...
        if self.deck_preview then
            self.deck_preview:remove(); self.deck_preview = nil
        end
        if self.shop then
            self.shop:remove(); self.shop = nil
        end
        if self.blind_select then
            self.blind_select:remove(); self.blind_select = nil
        end
        if self.booster_pack then
            self.booster_pack:remove(); self.booster_pack = nil
        end
        if self.MAIN_MENU_UI then
            self.MAIN_MENU_UI:remove(); self.MAIN_MENU_UI = nil
        end
        if self.SPLASH_FRONT then
            self.SPLASH_FRONT:remove(); self.SPLASH_FRONT = nil
        end
        if self.SPLASH_BACK then
            self.SPLASH_BACK:remove(); self.SPLASH_BACK = nil
        end
        if self.SPLASH_LOGO then
            self.SPLASH_LOGO:remove(); self.SPLASH_LOGO = nil
        end
        if self.GAME_OVER_UI then
            self.GAME_OVER_UI:remove(); self.GAME_OVER_UI = nil
        end
        if self.collection_alert then
            self.collection_alert:remove(); self.collection_alert = nil
        end
        if self.HUD then
            self.HUD:remove(); self.HUD = nil
        end
        if self.HUD_blind then
            self.HUD_blind:remove(); self.HUD_blind = nil
        end
        ...
        if self.OVERLAY_MENU then
            self.OVERLAY_MENU:remove(); self.OVERLAY_MENU = nil
        end
...
function Game:sandbox()
    ...
    if G.SPLASH_FRONT then
        G.SPLASH_FRONT:remove(); G.SPLASH_FRONT = nil
    end
    if G.SPLASH_BACK then
        G.SPLASH_BACK:remove(); G.SPLASH_BACK = nil
    end

    G.SPLASH_BACK = Sprite(-30, -13, G.ROOM.T.w + 60, G.ROOM.T.h + 22,
        G.ASSET_ATLAS["ui_" .. (G.SETTINGS.colourblind_option and 2 or 1)], { x = 2, y = 0 })
    ...
    function create_UIBox_sandbox_controls()
        ...
        G.FUNCS.spawn_joker = function(e)
            G.FUNCS.rem_joker(); G.SANDBOX.joker = add_joker(G.SANDBOX.joker_text, G.SANDBOX.edition)
        end
        G.FUNCS.rem_joker = function(e) if G.SANDBOX.joker then
                G.SANDBOX.joker:remove(); G.SANDBOX.joker = nil
            end end
        ...
        G.FUNCS.cb = function(rt)
            G.CARD_W = rt.ref_table[rt.ref_value] * G.SANDBOX.base_size.w; G.CARD_H = rt.ref_table[rt.ref_value] *
            G.SANDBOX.base_size.h
        end
            ...
            nodes = {
                {
                    ...
                    nodes = {
                        create_slider({ label = 'Time', w = 2, h = 0.3, text_scale = 0.2, label_scale = 0.3, ref_table =
                        G.SANDBOX, ref_value = 'vort_time', min = 0, max = 30 }),
                        create_option_cycle({ options = { 'PLAY', 'PAUSE' }, opt_callback = 'do_time', current_option = 1, colour =
                        G.C.RED, w = 2, scale = 0.7 }),
                        create_slider({ label = 'tilt', w = 2, h = 0.3, text_scale = 0.2, label_scale = 0.3, ref_table =
                        G.SANDBOX, ref_value = 'tilt', min = 0, max = 3, decimal_places = 2 }),
                        create_slider({ label = 'Card size', w = 2, h = 0.3, text_scale = 0.2, label_scale = 0.3, ref_table =
                        G.SANDBOX, ref_value = 'card_size', min = 0.1, max = 3, callback = 'cb', decimal_places = 2 }),
                        create_option_cycle({ options = G.SANDBOX.col_op, opt_callback = 'col1change', current_option = 1, colour =
                        G.C.RED, w = 2, scale = 0.7 }),
                        create_option_cycle({ options = G.SANDBOX.col_op, opt_callback = 'col2change', current_option = 2, colour =
                        G.C.RED, w = 2, scale = 0.7 }),
                        {
                            n = G.UIT.R,
                            ...
                            nodes = {
                                ...
                                create_text_input({ prompt_text = 'Joker key', extended_corpus = true, ref_table = G
                                .SANDBOX, ref_value = 'joker_text', text_scale = 0.3, w = 1.5, h = 0.6 }),
                                UIBox_button { label = { "-" }, button = "rem_joker", minw = 0.7, col = true },
                            }
                        },
                        create_option_cycle({ options = { 'base', 'foil', 'holo', 'polychrome', 'negative' }, opt_callback =
                        'edition_change', current_option = 1, colour = G.C.RED, w = 2, scale = 0.7 }),
                    }
                }
            }
        }
...
function Game:main_menu(change_context) --True if main menu is accessed from the splash screen, false if it is skipped or accessed from the game
    ...
    if (not G.SETTINGS.tutorial_complete) and G.SETTINGS.tutorial_progress.completed_parts['big_blind'] then
        set_cloud_flag('tutorial_complete') end
    ...
    local special_main_menu = G.ACHIEVEMENTS.completionist_plus_plus.earned and 3 or
    G.ACHIEVEMENTS.completionist_plus.earned and 2 or G.ACHIEVEMENTS.completionist.earned and 1 or 0
    ...
    local replace_card = Card(self.title_top.T.x, self.title_top.T.y, 1.2 * G.CARD_W * SC_scale, 1.2 * G.CARD_H *
    SC_scale, G.P_CARDS.S_A, G.P_CENTERS.c_base)
    ...
    if replace_card and (G.P_CENTERS.j_blueprint.unlocked) then
        ...
        if #viable_unlockables > 0 then
            ...
            G.E_MANAGER:add_event(Event({
                ...
                delay = 4.04,
                func = (function()
                    card = Card(self.title_top.T.x, self.title_top.T.y, 1.2 * G.CARD_W * SC_scale, 1.2 * G.CARD_H *
                    SC_scale, nil, pseudorandom_element(viable_unlockables) or self.P_CENTERS.j_joker)
    ...
    G.E_MANAGER:add_event(Event({ func = function()
        G.CONTROLLER.lock_input = false; return true
    end }))
...
function Game:start_run(args)
    ...
    if not saveTable then
        ...
        ease_background_colour_blind(G.STATE,
            saveTable.BLIND.name:gsub("%s+", "") ~= '' and saveTable.BLIND.name or 'Small Blind')
    end
    ...
    (self.GAME.viewed_back and self.GAME.viewed_back.name) or self.GAME.selected_back and self.GAME.selected_back.name or
    'Red Deck'
    ...
    if not saveTable then
        ...
        (not (G.SETTINGS.tutorial_complete or G.SETTINGS.tutorial_progress.completed_parts['big_blind']) and "TUTORIAL") or
        generate_starting_seed()
    end

    for k, v in pairs(self.GAME.pseudorandom) do if v == 0 then self.GAME.pseudorandom[k] = pseudohash(k ..
            self.GAME.pseudorandom.seed) end end
    ...
    if not saveTable then
        ...
        self.GAME.current_round.voucher = G.SETTINGS.tutorial_progress and G.SETTINGS.tutorial_progress.forced_voucher or
        get_next_voucher_key()
        self.GAME.round_resets.blind_tags.Small = G.SETTINGS.tutorial_progress and
        G.SETTINGS.tutorial_progress.forced_tags and G.SETTINGS.tutorial_progress.forced_tags[1] or get_next_tag_key()
        self.GAME.round_resets.blind_tags.Big = G.SETTINGS.tutorial_progress and G.SETTINGS.tutorial_progress
        .forced_tags and G.SETTINGS.tutorial_progress.forced_tags[2] or get_next_tag_key()
    else
    ...
    if saveTable then
        ...
        if not card_protos then
            ...
            for k, v in pairs(self.P_CARDS) do
                local _ = nil
                if self.GAME.starting_params.erratic_suits_and_ranks then _, k = pseudorandom_element(G.P_CARDS,
                        pseudoseed('erratic')) end
...
function Game:update(dt)
    ...
    if not G.fbf or G.new_frame then
        ...
        if not G.SETTINGS.paused then G.profile_display = (G.PROFILES and G.PROFILES[G.SETTINGS.profile] and G.PROFILES[G.SETTINGS.profile].name) or
            'ERROR' end
                    ...
                    self.E_MANAGER:add_event(Event({
                        func = function()
                            if _card and _card.area and _card.area == G.hand then
                                ...
                                G.CONTROLLER:update_cursor({ x = _x / (G.TILESCALE * G.TILESIZE), y = _y /
                ...
                G.E_MANAGER:add_event(Event({
                    ...
                    func = function()
                        if math.abs(G.shop.T.y - G.shop.VT.y) < 3 then
                            ...
                            if not nosave_shop then G.E_MANAGER:add_event(Event({ func = function()
...
function Game:update_async_load_screen(dt)
    if G.offline_screen then
        G.offline_screen:remove(); G.offline_screen = nil
    end
...
function Game:update_async_offline(dt)
    if G.loading_message then
        G.loading_message:remove(); G.loading_message = nil
    end
